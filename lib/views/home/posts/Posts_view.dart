...
// //                   userName: post['name'] ?? '',
// //                   userImage:
// //                       'assets/images/profile.png', // Replace with actual user image
...
// Widget _buildPosts(String status) {
//   return StreamBuilder<QuerySnapshot>(
//     stream: _firestore.collection('lostItems').orderBy('postTime').snapshots(),
//     builder: (context, snapshot) {
//       if (snapshot.hasData) {
//         List<Map<String, dynamic>> posts = snapshot.data!.docs
//             .map((doc) => {
...
//           future: DatabaseHelper().getImages(),
//           builder: (context, imageSnapshot) {
//             if (imageSnapshot.hasData) {
//               List<File> images = imageSnapshot.data as List<File>;
//               return ListView.builder(
//                 itemCount: posts.length,
//                 itemBuilder: (context, index) {
//                   final post = posts[index];
...
//                       postStatus: post['status'] ?? '',
//                       postText: post['description'] ?? '',
//                       postImage: images.length > index ? images[index].path : post['image'] ?? '',
//                       address: post['address'] ?? '',
//                       lostItem: post['lostItem'] ?? '',
//                       areaOfLossing: post['areaOfLossing'] ?? '',
//                       placementStation: post['placementStation'] ?? '',
...
//               child: Text('Claim'),
//               onPressed: () {
//                 // Add logic to claim the item here
//                 // For example, you can update the post status to 'claimed'
//                 // and add the police case number to the post data
//                 Navigator.of(context).pop();
//               },
//             ),
//           ],
...
//               Text('Post Image: ${widget.postImage}'),
//             ],
//           ),
//           actions: [
//             TextButton(
...
class _PostCardState extends State<PostCard> {
      ...
      child: Card(
        ...
        child: Column(
          ...
          children: [
            ...
            //               return Icon(Icons.image, size: 100, color: Colors.grey); // Placeholder icon
            //             },
            //           )
            //         : Icon(Icons.image, size: 100, color: Colors.grey), // Placeholder when no image path
            //   ),
            // ),
            // Interaction Icons and Status
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      (widget.postStatus == 'found' &&
                              widget.currentUserCategory == 'User' && widget.ownerId == FirebaseAuth.instance.currentUser!.uid)
                          ? ElevatedButton(
                              onPressed: () {
                                _showClaimDialog();
                              },
  ...
  //             child: Text('Claim'),
  //             onPressed: () async {
  //               final caseNumber = _policeCaseNumberController.text;
  ...
  //                 ScaffoldMessenger.of(context).showSnackBar(
  //                   SnackBar(content: Text('Invalid case number')),
  //                 );
  //               } else {
